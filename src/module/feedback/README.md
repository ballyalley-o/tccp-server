# Feedback Module Structure Guideline
...
feedback.schema.ts
feedback.type.ts
feedback.constant.ts
feedback.permission.ts
```

For sub-features, keep the entity name first.

```txt
feedback.blah.controller.ts
```
...
If a helper is truly shared by many modules, move it to `src/common/util`. If it is only used by `feedback`, name it after the behavior.

```txt
build-feedback-filter.ts
normalize-feedback-order.ts
assert-feedback-category-exists.ts
```
...
- permission-aware decisions that are specific to feedback behavior
...
```ts
import { SkillController } from '@module/feedback'
```

Avoid deep imports from another module.

```ts
// Avoid this from outside the feedback module.
import SkillController from '@module/feedback/feedback/controller/feedback.controller'
```

Inside the feedback module, local relative imports are acceptable when they stay close and readable.

```ts
import { createSkill } from './feedback.service'
```
...
Keep these inside `src/module/feedback`:

- feedback routes
- feedback controllers
- feedback services
- feedback category routes
- feedback category controllers
- feedback category services
- feedback-specific validation
- feedback-specific permissions
- feedback-specific query builders
...
```ts
export { default as skillRoute } from './feedback/feedback.route'
export { default as skillCategoryRoute } from './category/feedback-category.route'
export { default as SkillController } from './feedback/feedback.controller'
export { default as SkillCategoryController } from './category/feedback-category.controller'
```
...
When adding a new feedback endpoint:
