...
import { SCHEMA, USER_STATUS, DEFAULT_USER_STATUS } from '@common/constant'
import { REGEX }                                    from '@common/constant/regex'
import { oneDayFromNow }                            from '@common/constant/max-age'

import AdminAudit from '@module/admin/admin.audit/model/AdminAudit'
const TAG = MODULE_KEY.AUTH_USER
...
AuthUserSchema.methods.getSignedJwtToken = function () {
  return jwt.sign({ id: this._id, tokenVersion: this.tokenVersion }, GLOBAL.JWT_SECRET || '', {
    expiresIn: GLOBAL.JWT_EXP
  })
}

AuthUserSchema.methods.matchPassword = async function (enteredPassword: string) {
...
AuthUserSchema.index(DATABASE_INDEX.USER)

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
AuthUserSchema.add(AdminAudit.obj)

const User = model(TAG, AuthUserSchema)
export default User
