...
import { RESPONSE, thirtyDaysFromNow, fiveSecFromNow, expire, AUTH_KEY } from '@common/constant'
import { Code }                                                          from '@common/constant/enum'
import { ErrorResponse, htmlContent, sendEmail }                         from '@common/util'
...
class AuthController {
   ...
   * @returns void
   */
  private static _sendTokenResponse = (user: IAuthUser, statusCode: number, res: any) => {
  ...
  public static async register(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const [emailExists, usernameExists] = await Promise.all([
        AuthUser.findOne({ email }).lean(),
        AuthUser.findOne({ username }).lean()
      ])
  ...
  public static async login(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const user = await AuthUser.findOne({ email, archivedAt: null }).select('+password')
  ...
  public static async myAccount(req: any, res: Response, _next: NextFunction) {
    try {
      const user = (await AuthUser.findById(req.user.id)) || null
  ...
  public static async updateAccount(req: Request, res: Response, _next: NextFunction) {
    ...
    try {
      const user = await AuthUser.findByIdAndUpdate(userId, fieldsToUpdate, {
        new          : true,
        runValidators: true
      })
