...
import { CourseEvent }                                                from '@module/course/course.event'
import { AuthUser }                                                   from '@module/auth/auth.user'
import { DashboardService }                                           from '@module/dashboard'
import { use, LogRequest }                                            from '@common/decorator'
import { AUTH_KEY, RESPONSE, LOCALE,  COURSE_EVENT, CourseEventType } from '@common/constant'
...
class DashboardController {
  private static async getAuthenticatedUser(req: Request) {
    ...
    try {
      ...
      if (!user) {
        user = await AuthUser.findById(decoded.id).select('-password').lean()
        if (user) {
  ...
  private static getAudience(req: Request, authUser: any): AppUserRoleExtendedType {
    const roleQuery = String(req.query.role ?? '').toLowerCase()

    if (roleQuery === 'guest' || roleQuery === 'user' || roleQuery === 'trainer' || roleQuery === 'admin') {
      return roleQuery as AppUserRoleExtendedType
    }
  ...
  private static getLocale(req: Request): string {
    ...
    return Object.values(LOCALE).includes(localeQuery as AppLocaleLangType) ? localeQuery : LOCALE.EN
  }
  ...
  private static buildActivitySeries(events: any[], locale = 'en', periodDays = 7) {
    ...
    const trendValue = previousTotal === 0 ? (total > 0 ? 100 : 0) : Math.round(((total - previousTotal) / previousTotal) * 100)
  ...
  private static buildGuestDashboard(
    ...
    recommendations  : DashboardRecommendationType[],
    featured         : any[],
    activityData     : any,
    skillDistribution: Array<{ label: string; value: number }>
  ) {
    return {
      audience            : 'guest',
  ...
  private static buildUserDashboard(
    counts           : DashboardCountSummaryType,
    recommendations  : DashboardRecommendationType[],
    featured         : any[],
    enrollments      : any[],
    ...
    const progressValue    = enrollments.length
    ...
    return {
      ...
      stat                : [
        ...
        {
          id      : 'streak',
          label   : 'dashboard.study_streak',
          value   : activeCourses ? activeCourses + 1: 1,
          tone    : 'warning',
          subtitle: 'dashboard.days_row'
        },
      ...
      skillDistribution,
      distribution: skillDistribution,
      course      : courseItems as DashboardCourseType[]
    }
  }

  private static buildTrainerDashboard(
    counts           : DashboardCountSummaryType,
    recommendations  : DashboardRecommendationType[],
    featured         : any[],
    trainerBootcamps : any[],
    studentCount     : number,
    activityData     : any,
    skillDistribution: Array<{ label: string; value: number }>
  ) {
    ...
    return {
      ...
      stat                : [
        {
          id      : 'active-courses',
          label   : 'dashboard.active_courses',
          value   : trainerBootcamps.length || 0,
          tone    : 'primary',
          subtitle: 'dashboard.published_now'
        },
  ...
  private static buildAdminDashboard(
    counts           : DashboardCountSummaryType,
    recommendations  : DashboardRecommendationType[],
    featured         : any[],
    activityData     : any,
    ...
    try {
      ...
      if (!eventType || typeof eventType !== 'string') {
        return res.status(Code.BAD_REQUEST).json({
          success: false,
          message: RESPONSE.error.IS_REQUIRED('eventType')
        })
      }

      if (!COURSE_EVENT.includes(eventType as CourseEventType)) {
        return res.status(Code.BAD_REQUEST).json({
          success: false,
      ...
      const learningEvent = await CourseEvent.create({
        user    : req.user?._id,
        course  : courseId,
        bootcamp: bootcampId,
        eventType,
  ...
  public static async getDashboard(req: Request, res: Response) {
    try {
      const [userCount, trainerCount, adminCount, bootcampCount, courseCount, feedbackCount] = await Promise.all([
        ...
        AuthUser.countDocuments({ role: 'trainer' }),
        AuthUser.countDocuments({ role: 'admin' }),
      ...
      const dashboardData = await (async () => {
        if (audience === 'user' && authUser) {
          ...
          const userEvents        = await CourseEvent.find({ user: authUser._id, occurredAt: { $gte: activityWindowStart } }).lean()
          const activityData      = DashboardController.buildActivitySeries(userEvents, locale)
          const skillDistribution = DashboardService.buildSkillDistribution(
        ...
        if (audience === 'trainer' && authUser) {
          ...
          const platformEvents    = await CourseEvent.find({ occurredAt: { $gte: activityWindowStart } }).lean()
          const activityData      = DashboardController.buildActivitySeries(platformEvents, locale)
          const skillDistribution = DashboardService.buildSkillDistribution(platformCourses.map((course) => ({ course })))
